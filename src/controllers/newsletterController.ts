...
import { NewsletterSubscribeSchema } from "@/models";
import { checkRateLimit, getClientIp, RATE_LIMIT_PROFILES } from "@/lib/rateLimit";
import { db } from "@/db/client";

/**
 * Newsletter Controller
 * Orchestrates newsletter subscription, validation, and subscriber persistence.
 */
export async function handleNewsletterSubscription(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[newsletterController] Error subscribing to newsletter:", error);
    return NextResponse.json(
