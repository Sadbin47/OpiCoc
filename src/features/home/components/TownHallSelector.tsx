  ...
  return (
    <section className="py-16 sm:py-24 border-b border-[#262B35] bg-[#0E1117]">
      <Container size="default">
        ...
        {/* 4-Column Town Hall Tier Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {TOWN_HALL_TIERS.map((tier) => (
            <Link
              key={tier.level}
              href={`/all-products?th=${tier.level}`}
              className="group relative flex flex-col justify-between rounded-xl border border-[#262B35] bg-[#12151B] p-5 transition-all duration-300 hover:-translate-y-1.5 hover:border-amber-500/50 hover:bg-[#161B22] hover:shadow-2xl hover:shadow-amber-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500"
            >
                ...
                <div className="relative w-full h-40 flex items-center justify-center my-3 select-none">
                  ...
                  <div className="relative w-36 h-36">
                    <Image
                      ...
                      className="object-contain filter drop-shadow-[0_12px_18px_rgba(0,0,0,0.7)] group-hover:scale-108 group-hover:rotate-1 transition-all duration-300"
                    />
                  </div>
                </div>
